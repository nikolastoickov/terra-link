  ...
  return (
    <header className="sticky top-0 z-50 border-b border-navy/10 bg-cream/80 shadow-sm shadow-navy/[0.03] backdrop-blur-md">
      <div className="relative mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
        ...
        <nav className="absolute left-1/2 hidden -translate-x-1/2 items-center gap-8 text-sm font-medium text-navy lg:flex">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
        ...
        <a
          ...
          className="hidden items-center gap-2 rounded-full bg-gold px-5 py-2.5 text-navy shadow-sm shadow-gold/40 transition hover:-translate-y-0.5 hover:shadow-md hover:shadow-gold/40 hover:brightness-95 lg:inline-flex"
        >
          <svg
            width="16"
            height="16"
        ...
        <a
          href="tel:+381641112104"
          className="inline-flex items-center gap-1.5 rounded-full bg-gold px-4 py-2 text-sm font-semibold text-navy shadow-sm shadow-gold/40 transition active:scale-95 lg:hidden"
        >
          <svg
            width="14"
            height="14"
